...
function createChatMessageItems(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const threadCreatedFromMessage = messageInfo.id
...
function useMessageListData({
  ...
  const pendingSidebarSourceMessageInfo = useSelector(state => {
    if (!threadIsPendingSidebar(threadInfo) || !sourceMessageID) {
      return null;
    }
    const realizedSidebar = threadInfoFromSourceMessageIDSelector(state)[
      sourceMessageID
    ];
    if (realizedSidebar) {
      return null;
    }
    return messageInfoSelector(state)[sourceMessageID];
  });
