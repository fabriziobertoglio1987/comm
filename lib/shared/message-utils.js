...
function sortMessageInfoList<T: MessageInfo | RawMessageInfo>(
  ...
  return _orderBy(['time', 'id'])(['desc', 'desc'])(messageInfos);
}
...
function createMediaMessageInfo(
  ...
  const createRawMessageInfo = messageSpecs[messageData.type].rawMessageInfoFromMessageData;
  invariant(
    createRawMessageInfo,
    'multimedia message spec should have rawMessageInfoFromMessageData');
  return createRawMessageInfo(messageData, input.id);
}
