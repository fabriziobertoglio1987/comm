...
function threadIsPersonalAndPending(threadInfo: ?(ThreadInfo | RawThreadInfo)): boolean {
  return (
    threadInfo?.type === threadTypes.PERSONAL && threadIsPending(threadInfo?.id)
  );
}

function threadIsPendingSidebar(threadInfo: ?ThreadInfo): boolean {
  return (
    threadInfo?.type === threadTypes.SIDEBAR && threadIsPending(threadInfo?.id)
  );
}

function getPendingThreadOtherUsers(
  threadInfo: ThreadInfo | RawThreadInfo,
  viewerID: ?string,
): string[] {
...
function getPendingThreadID(
  ...
  const pendingThreadKey = sourceMessageID
    ...
    : [...memberIDs].sort().join('+');
  return `pending/${pendingThreadKey}`;
}
...
function createPendingThread({
  ...
  const threadID = getPendingThreadID(memberIDs, sourceMessageID);
...
function pendingThreadType(numberOfOtherMembers: number): 4 | 6 {
...
async function createRealThreadFromPendingThread({
  ...
  const otherMemberIDs = getPendingThreadOtherUsers(threadInfo, viewerID);
  try {
    let resultPromise;
...
function useCurrentThreadInfo({
  ...
  userInfoInputArray,
  sourceMessageID,
}: UseCurrentThreadInfoArgs): ?ThreadInfo {
